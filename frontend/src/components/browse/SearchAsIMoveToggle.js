...
export default function SearchAsIMoveToggle({ checked, onChange, isFetching }) {
  return (
    <div className="absolute left-1/2 top-3 z-10 -translate-x-1/2">
      <label className="flex cursor-pointer select-none items-center gap-2 rounded-full border border-sectionBorder bg-white px-3 py-1.5 text-xs font-medium text-foreground shadow-md hover:bg-card">
        <input
          type="checkbox"
          checked={checked}
